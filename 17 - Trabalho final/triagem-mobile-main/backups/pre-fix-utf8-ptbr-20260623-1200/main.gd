...
const BACKEND_PENDING_REVIEW_ENDPOINT: String = BACKEND_BASE_URL + "/triages/pending-review"
...
func _ready() -> void:
	...
	status_label.text = "Paciente: vÃ¡ ao totem da recepÃ§Ã£o e pressione Interagir."
...
func _toggle_player() -> void:
	...
	if active_player == patient:
		...
		status_label.text = "Enfermeiro: vÃ¡ ao computador da recepÃ§Ã£o para analisar triagens."
	else:
		_set_active_player(patient)
		status_label.text = "Paciente: vÃ¡ ao totem da recepÃ§Ã£o para registrar sintomas."
...
func _interact() -> void:
	...
	if active_player.role == "patient":
		...
		status_label.text = "Enfermeiro precisa estar perto do computador da recepcao."
...
func _open_review_panel() -> void:
	...
	review_text.text = "[center]Carregando triagens aguardando revisao...[/center]"
	confirm_button.disabled = true
	status_label.text = "Buscando triagens em andamento no backend..."
...
func _confirm_triage() -> void:
	if pending_triage.is_empty():
		status_label.text = "NÃ£o hÃ¡ triagem pendente para confirmar."
		return
	...
	status_label.text = "Confirmando revisÃ£o profissional no backend..."
	var url: String = BACKEND_TRIAGES_ENDPOINT + "/" + str(triage_id) + "/professional-review"
	var error: Error = _request_json(url, HTTPClient.METHOD_PATCH, payload)
	if error != OK:
		status_label.text = "Erro ao iniciar confirmaÃ§Ã£o da triagem."
...
func _handle_http_error(response_code: int, data) -> void:
	...
	if current_request == "fetch_pending_review":
...
func _handle_create_triage_success(data) -> void:
	symptom_panel.visible = false
	status_label.text = "Triagem enviada. Aguarde o processamento da IA e a revisÃ£o do enfermeiro."
...
func _handle_pending_review_success(data) -> void:
	...
	confirmed_message = ""
	_render_review()
	status_label.text = "Triagem carregada para revisÃ£o do enfermeiro."
